...
/// <summary>
/// Hosted сервис, который последовательно вызывает <see cref="DoExecuteTaskAsync"/> через паузу <see cref="RepeatingHostedService.Interval"/>
/// Обеспечивает запись данных в систему auditTrail
/// Как использовать:
/// - создать класс наследник <see cref="MoedeloRepeatingHostedService"/>
/// - реализовать метод <see cref="DoExecuteTaskAsync"/> в классе наследнике
/// - для изменения периода вызова метода <see cref="DoExecuteTaskAsync"/> следует переопределить свойство <see cref="RepeatingHostedService.Interval"/>
/// </summary>
public abstract class MoedeloRepeatingHostedService : RepeatingHostedService
{
    private readonly IAuditTracer auditTracer;
    private readonly string auditSpanName;

    protected MoedeloRepeatingHostedService(
