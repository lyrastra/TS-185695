...
{
    [InjectAsSingleton(typeof(ILoanIssueCommandWriter))]
    internal sealed class LoanIssueCommandWriter : ILoanIssueCommandWriter
    {
        private const string EntityName = MoneyTopics.PaymentOrders.LoanIssue.EntityName;
        private static readonly string Topic = MoneyTopics.PaymentOrders.LoanIssue.Command.Topic;
        ...
        public Task WriteImportDuplicateAsync(
            ImportDuplicateLoanIssue commandData)
        {
            var context = executionInfoContextAccessor.ExecutionInfoContext;
            var commandDefinition = definitionBuilder.CreateCommandDefinition(context.FirmId.ToString(), commandData);
            return commandKafkaTopicWriter.WriteCommandDataAsync(commandDefinition);
        ...
        public Task WriteImportWithMissingContractAsync(
            ImportWithMissingContractLoanIssue commandData)
        {
            var context = executionInfoContextAccessor.ExecutionInfoContext;
            var commandDefinition = definitionBuilder.CreateCommandDefinition(context.FirmId.ToString(), commandData);
            return commandKafkaTopicWriter.WriteCommandDataAsync(commandDefinition);
        ...
        public Task WriteImportWithMissingContractorAsync(
            ImportWithMissingContractorLoanIssue commandData)
        {
            var context = executionInfoContextAccessor.ExecutionInfoContext;
            var commandDefinition = definitionBuilder.CreateCommandDefinition(context.FirmId.ToString(), commandData);
            return commandKafkaTopicWriter.WriteCommandDataAsync(commandDefinition);
