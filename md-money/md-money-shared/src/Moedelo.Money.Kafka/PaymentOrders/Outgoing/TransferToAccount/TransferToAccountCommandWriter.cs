...
{
    [InjectAsSingleton(typeof(ITransferToAccountCommandWriter))]
    internal sealed class TransferToAccountCommandWriter : ITransferToAccountCommandWriter
    {
        private const string EntityName = MoneyTopics.PaymentOrders.TransferToAccount.EntityName;
        private static readonly string Topic = MoneyTopics.PaymentOrders.TransferToAccount.Command.Topic;
        ...
        public Task WriteImportAsync(ImportTransferToAccount commandData)
        {
        ...
        public Task WriteImportDuplicateAsync(ImportDuplicateTransferToAccount commandData)
        {
            var context = executionInfoContextAccessor.ExecutionInfoContext;
            var commandDefinition = definitionBuilder.CreateCommandDefinition(context.FirmId.ToString(), commandData);
            return commandKafkaTopicWriter.WriteCommandDataAsync(commandDefinition);
        ...
        public Task WriteApplyIgnoreNumberAsync(ApplyIgnoreNumberTransferToAccount commandData)
        {
            var context = executionInfoContextAccessor.ExecutionInfoContext;
            var commandDefinition = definitionBuilder.CreateCommandDefinition(context.FirmId.ToString(), commandData);
            return commandKafkaTopicWriter.WriteCommandDataAsync(commandDefinition);
