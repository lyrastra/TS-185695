...
{
    [InjectAsSingleton(typeof(ICurrencyBankFeeEventReader))]
    internal sealed class CurrencyBankFeeEventReader : MoedeloKafkaTopicReaderBase, ICurrencyBankFeeEventReader
    {
        public CurrencyBankFeeEventReader(
            IMoedeloKafkaTopicReaderBaseDependencies dependencies,
            ILogger<CurrencyBankFeeEventReader> logger)
            : base(
                dependencies,
                logger)
        {
        ...
        public Task ReadAsync(string groupId,
            Func<CurrencyBankFeeCreatedMessage, KafkaMessageValueMetadata, Task> onCreate,
            Func<CurrencyBankFeeUpdatedMessage, KafkaMessageValueMetadata, Task> onUpdate,
            Func<CurrencyBankFeeDeletedMessage, KafkaMessageValueMetadata, Task> onDelete,
            ...
            CancellationToken? cancellationToken = null)
        {
            ...
            Task OnMessageAction(CUDEventMessageValue messageValue)
            {
                ...
                switch (messageValue.EventType)
                {
                    case CUDEventType.Created:
                        var createdMessage = messageValue.EventDataJson.FromJsonString<CurrencyBankFeeCreatedMessage>();
                        return onCreate(createdMessage, metadata);
                    case CUDEventType.Updated:
                        var updatedMessage = messageValue.EventDataJson.FromJsonString<CurrencyBankFeeUpdatedMessage>();
                        return onUpdate(updatedMessage, metadata);
                    case CUDEventType.Deleted:
                        var deletedMessage = messageValue.EventDataJson.FromJsonString<CurrencyBankFeeDeletedMessage>();
            ...
            var topicName = PaymentOrderTopics.CurrencyBankFee.Event.CUD;
