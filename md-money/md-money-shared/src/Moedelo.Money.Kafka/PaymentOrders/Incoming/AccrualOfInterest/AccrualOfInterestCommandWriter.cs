...
{
    [InjectAsSingleton(typeof(IAccrualOfInterestCommandWriter))]
    internal sealed class AccrualOfInterestCommandWriter : IAccrualOfInterestCommandWriter
    {
        private const string EntityName = MoneyTopics.PaymentOrders.AccrualOfInterest.EntityName;
        private static readonly string Topic = MoneyTopics.PaymentOrders.AccrualOfInterest.Command.Topic;
        ...
        public Task WriteImportAsync(ImportAccrualOfInterest commandData)
        {
        ...
        public Task WriteImportDuplicateAsync(ImportDuplicateAccrualOfInterest commandData)
        {
            var context = executionInfoContextAccessor.ExecutionInfoContext;
            var commandDefinition = definitionBuilder.CreateCommandDefinition(context.FirmId.ToString(), commandData);
            return commandKafkaTopicWriter.WriteCommandDataAsync(commandDefinition);
