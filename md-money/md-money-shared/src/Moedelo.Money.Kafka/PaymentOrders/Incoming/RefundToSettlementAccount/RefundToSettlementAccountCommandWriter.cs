...
{
    [InjectAsSingleton(typeof(IRefundToSettlementAccountCommandWriter))]
    public class RefundToSettlementAccountCommandWriter : IRefundToSettlementAccountCommandWriter
    {
        private const string EntityName = MoneyTopics.PaymentOrders.RefundToSettlementAccount.EntityName;
        private static readonly string Topic = MoneyTopics.PaymentOrders.RefundToSettlementAccount.Command.Topic;
        ...
        public RefundToSettlementAccountCommandWriter(IMoedeloEntityCommandKafkaTopicWriter commandKafkaTopicWriter,
        ...
        public Task WriteImportAsync(ImportRefundToSettlementAccount commandData)
        {
        ...
        public Task WriteImportDuplicateAsync(ImportRefundToSettlementAccountDuplicate commandData)
        {
            var context = executionInfoContextAccessor.ExecutionInfoContext;
            var commandDefinition = definitionBuilder.CreateCommandDefinition(context.FirmId.ToString(), commandData);
            return commandKafkaTopicWriter.WriteCommandDataAsync(commandDefinition);
        ...
        public Task WriteImportWithMissingContractorAsync(ImportRefundToSettlementAccountWithMissingContragent commandData)
        {
            var context = executionInfoContextAccessor.ExecutionInfoContext;
            var commandDefinition = definitionBuilder.CreateCommandDefinition(context.FirmId.ToString(), commandData);
            return commandKafkaTopicWriter.WriteCommandDataAsync(commandDefinition);
