...
{
    [InjectAsSingleton(typeof(ITransferFromAccountCommandWriter))]
    internal sealed class TransferFromAccountCommandWriter : ITransferFromAccountCommandWriter
    {
        private const string EntityName = MoneyTopics.PaymentOrders.TransferFromAccount.EntityName;
        private static readonly string Topic = MoneyTopics.PaymentOrders.TransferFromAccount.Command.Topic;
        ...
        public Task WriteImportAsync(ImportTransferFromAccount commandData)
        {
        ...
        public Task WriteImportDuplicateAsync(ImportDuplicateTransferFromAccount commandData)
        {
            var context = executionInfoContextAccessor.ExecutionInfoContext;
            var commandDefinition = definitionBuilder.CreateCommandDefinition(context.FirmId.ToString(), commandData);
            return commandKafkaTopicWriter.WriteCommandDataAsync(commandDefinition);
        ...
        public Task WriteImportAmbiguousOperationTypeAsync(ImportAmbiguousOperationTypeTransferFromAccount commandData)
        {
            var context = executionInfoContextAccessor.ExecutionInfoContext;
            var commandDefinition = definitionBuilder.CreateCommandDefinition(context.FirmId.ToString(), commandData);
            return commandKafkaTopicWriter.WriteCommandDataAsync(commandDefinition);
