...
{
    [InjectAsSingleton(typeof(IOtherIncomingEventReader))]
    internal sealed class OtherIncomingEventReader : MoedeloKafkaTopicReaderBase, IOtherIncomingEventReader
    {
        public OtherIncomingEventReader(
            IMoedeloKafkaTopicReaderBaseDependencies dependencies,
            ILogger<OtherIncomingEventReader> logger)
            : base(
                dependencies,
                logger)
        {
            ...
            CancellationToken? cancellationToken = null)
        {
            ...
            Task OnMessageAction(CUDEventMessageValue messageValue)
            {
                ...
                switch (messageValue.EventType)
                {
                    case CUDEventType.Created:
                        var createdMessage = messageValue.EventDataJson.FromJsonString<OtherIncomingCreatedMessage>();
                        return onCreate(createdMessage, metadata);
                    case CUDEventType.Updated:
                        var updatedMessage = messageValue.EventDataJson.FromJsonString<OtherIncomingUpdatedMessage>();
                        return onUpdate(updatedMessage, metadata);
                    case CUDEventType.Deleted:
                        var deletedMessage = messageValue.EventDataJson.FromJsonString<OtherIncomingDeletedMessage>();
            ...
            var topicName = PaymentOrderTopics.OtherIncoming.Event.CUD;
