...
{
    [InjectAsSingleton(typeof(ITransferFromPurseCommandWriter))]
    internal sealed class TransferFromPurseCommandWriter : ITransferFromPurseCommandWriter
    {
        private const string EntityName = MoneyTopics.PaymentOrders.TransferFromPurse.EntityName;
        private static readonly string Topic = MoneyTopics.PaymentOrders.TransferFromPurse.Command.Topic;
        ...
        public Task WriteImportAsync(ImportTransferFromPurse commandData)
        {
        ...
        public Task WriteImportDuplicateAsync(ImportDuplicateTransferFromPurse commandData)
        {
            var context = executionInfoContextAccessor.ExecutionInfoContext;
            var commandDefinition = definitionBuilder.CreateCommandDefinition(context.FirmId.ToString(), commandData);
            return commandKafkaTopicWriter.WriteCommandDataAsync(commandDefinition);
