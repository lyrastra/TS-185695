...
{
    [InjectAsSingleton(typeof(ILoanObtainingCommandWriter))]
    internal sealed class LoanObtainingCommandWriter : ILoanObtainingCommandWriter
    {
        private const string EntityName = MoneyTopics.PaymentOrders.LoanObtaining.EntityName;
        private static readonly string Topic = MoneyTopics.PaymentOrders.LoanObtaining.Command.Topic;
        ...
        public Task WriteImportAsync(
            ImportLoanObtaining commandData)
        {
        ...
        public Task WriteImportDuplicateAsync(
            ImportDuplicateLoanObtaining commandData)
        {
            var context = executionInfoContextAccessor.ExecutionInfoContext;
            var commandDefinition = definitionBuilder.CreateCommandDefinition(context.FirmId.ToString(), commandData);
            return commandKafkaTopicWriter.WriteCommandDataAsync(commandDefinition);
        ...
        public Task WriteImportWithMissingContractAsync(
            ImportWithMissingContractLoanObtaining commandData)
        {
            var context = executionInfoContextAccessor.ExecutionInfoContext;
            var commandDefinition = definitionBuilder.CreateCommandDefinition(context.FirmId.ToString(), commandData);
            return commandKafkaTopicWriter.WriteCommandDataAsync(commandDefinition);
        ...
        public Task WriteImportWithMissingContractorAsync(
            ImportWithMissingContractorLoanObtaining commandData)
        {
            var context = executionInfoContextAccessor.ExecutionInfoContext;
            var commandDefinition = definitionBuilder.CreateCommandDefinition(context.FirmId.ToString(), commandData);
            return commandKafkaTopicWriter.WriteCommandDataAsync(commandDefinition);
