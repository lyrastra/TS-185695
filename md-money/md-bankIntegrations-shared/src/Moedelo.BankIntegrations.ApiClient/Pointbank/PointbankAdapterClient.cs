...
namespace Moedelo.BankIntegrations.ApiClient.Pointbank
{
    ...
    public class PointbankAdapterClient : BaseApiClient, IPointbankAdapterClient
    {
        ...
        public async Task<GetConsentsDto> GetAllParentConsentsAsync(int firmId)
        {
            var result = await GetAsync<ApiDataResult<GetConsentsDto>>(uri: $"/{ssoController}/GetAllParentConsents", queryParams: new { firmId });
            return result.data;
        }
