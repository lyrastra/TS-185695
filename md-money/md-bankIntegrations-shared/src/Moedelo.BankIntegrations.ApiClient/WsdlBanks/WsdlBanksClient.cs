...
using Moedelo.BankIntegrations.ApiClient.Abstractions.WsdlBanks;
...
{
    [InjectAsSingleton]
    public class WsdlBanksClient : BaseApiClient, IWsdlBanksClient
    {
        public WsdlBanksClient(
            ...
            ILogger<WsdlBanksClient> logger) : base(
                httpRequestExecuter,
                uriCreator,
                auditTracer,
                authHeadersGetter,
                auditHeadersGetter,
                settingRepository.Get("WsdlBanksAdapterEndpoint"),
                logger)
        {
        }
