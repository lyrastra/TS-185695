...
namespace Moedelo.AccountingStatements.Kafka.TradingFee
{
    [InjectAsSingleton(typeof(ITradingFeeAccountingStatementEventReader))]
    internal sealed class TradingFeeAccountingStatementEventReader : MoedeloKafkaTopicReaderBase, ITradingFeeAccountingStatementEventReader
    {
        public TradingFeeAccountingStatementEventReader(
            IMoedeloKafkaTopicReaderBaseDependencies dependencies,
            ILogger<TradingFeeAccountingStatementEventReader> logger)
            : base(
                dependencies,
                logger)
        {
            ...
            CancellationToken? cancellationToken = null)
        {
            ...
            Task OnMessageAction(CDEventMessageValue messageValue)
            {
                ...
                switch (messageValue.EventType)
                {
                    case AccountingStatementEventType.Created:
                        var createdMessage = messageValue.EventDataJson.FromJsonString<TradingFeeCreatedMessage>();
                        return onCreateAsync(createdMessage, metadata);
                    case AccountingStatementEventType.Deleted:
                        var deletedMessage = messageValue.EventDataJson.FromJsonString<TradingFeeDeletedMessage>();
            ...
            var settings = new ReadTopicSetting<CDEventMessageValue>(
                groupId,
                AccountingStatementsTopics.Event.TradingFeeCD,
                OnMessageAction,
                onException,
                null,
                resetType: resetType,
