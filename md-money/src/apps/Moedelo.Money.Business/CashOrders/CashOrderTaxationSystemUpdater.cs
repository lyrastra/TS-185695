...
{
    [InjectAsSingleton(typeof(ICashOrderTaxationSystemUpdater))]
    internal class CashOrderTaxationSystemUpdater : ICashOrderTaxationSystemUpdater
    {
        ...
        public CashOrderTaxationSystemUpdater(
            ILogger<CashOrderTaxationSystemUpdater> logger,
        ...
        public async Task UpdateAsync(long documentBaseId, TaxationSystemType taxationSystemType, Guid guid)
        {
            ...
            {
                logger.LogWarning($"Unable to change taxation system for cash order with DocumentBaseId = {documentBaseId}. Operation not found");
            }
