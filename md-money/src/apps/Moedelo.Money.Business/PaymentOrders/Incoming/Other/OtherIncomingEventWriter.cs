...
namespace Moedelo.Money.Business.PaymentOrders.Incoming.Other
{
    ...
    [InjectAsSingleton(typeof(OtherIncomingEventWriter))]
    [InjectAsSingleton(typeof(IConcretePaymentOrderDeletedEventWriter))]
    internal sealed class OtherIncomingEventWriter : MoedeloKafkaTopicWriterBase, IConcretePaymentOrderDeletedEventWriter
    {
        private readonly string topic = PaymentOrderTopics.OtherIncoming.Event.CUD;

        public OtherIncomingEventWriter(
            IMoedeloKafkaTopicWriterBaseDependencies dependencies)
            : base(dependencies)
        {
        }

        public Task WriteCreatedEventAsync(OtherIncomingSaveRequest request)
        {
            var message = OtherIncomingMapper.MapToCreatedMessage(request);
        ...
        public Task WriteUpdatedEventAsync(OtherIncomingSaveRequest request)
        {
            var message = OtherIncomingMapper.MapToUpdatedMessage(request);
            var key = message.DocumentBaseId.ToString();
            var value = new CUDEventMessageValue
            {
        ...
        public Task WriteDeletedEventAsync(OtherIncomingResponse response, long? newDocumentBaseId)
        {
            var message = OtherIncomingMapper.MapToDeletedMessage(response, newDocumentBaseId);
            var key = message.DocumentBaseId.ToString();
            var value = new CUDEventMessageValue
            {
        ...
        {
            var message = new OtherIncomingDeletedMessage
            {
