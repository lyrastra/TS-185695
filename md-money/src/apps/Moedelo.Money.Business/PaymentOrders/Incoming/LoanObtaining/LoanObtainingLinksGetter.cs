...
{
    [InjectAsSingleton(typeof(LoanObtainingLinksGetter))]
    internal sealed class LoanObtainingLinksGetter
    {
        private readonly ILogger logger;
        private readonly ILinksReader linksReader;
        private readonly IContractsReader contractsReader;

        public LoanObtainingLinksGetter(
            ILogger<LoanObtainingLinksGetter> logger,
        ...
        public async Task<LoanObtainingLinks> GetAsync(long documentBaseId)
        {
            try
            {
                var links = await linksReader.GetLinksWithDocumentsAsync(documentBaseId);

                return new LoanObtainingLinks
                {
