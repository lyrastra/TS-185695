...
namespace Moedelo.Money.ChangeLog.HostedServices.CashOrders.Incoming
{
    ...
    {
        private static FinancialAssistanceStateDefinition StateDefinition =>
            FinancialAssistanceStateDefinition.Instance;
        ...
        protected override async Task ExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            await eventReaderBuilder
                .OnEvent<FinancialAssistanceCreated>(OnCreated)
                .OnEvent<FinancialAssistanceUpdated>(OnUpdated)
                .OnEvent<FinancialAssistanceDeleted>(OnDeleted)
                .SkipIfUnknownType()
        ...
        private async Task OnCreated(FinancialAssistanceCreated eventData, KafkaMessageValueMetadata eventMetadata)
        {
        ...
        private async Task OnUpdated(FinancialAssistanceUpdated eventData, KafkaMessageValueMetadata eventMetadata)
        {
            var extraInfo = await GetExtraStateInfoAsync(
                eventData.CashId);
            await changeLogCommandWriter.WriteAsync(
        ...
        private Task OnDeleted(FinancialAssistanceDeleted eventData, KafkaMessageValueMetadata eventMetadata)
        {
