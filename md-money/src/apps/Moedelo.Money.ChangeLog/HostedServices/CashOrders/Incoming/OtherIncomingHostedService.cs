    ...
    public class OtherIncomingHostedService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            await eventReaderBuilder
                .OnEvent<OtherIncomingCreated>(OnCreated)
                .OnEvent<OtherIncomingUpdated>(OnUpdated)
                .OnEvent<OtherIncomingDeleted>(OnDeleted)
                .SkipIfUnknownType()
        ...
        private async Task OnCreated(OtherIncomingCreated eventData, KafkaMessageValueMetadata eventMetadata)
        {
        ...
        private async Task OnUpdated(OtherIncomingUpdated eventData, KafkaMessageValueMetadata eventMetadata)
        {
            var extraInfo = await GetExtraStateInfoAsync(
                eventData.CashId,
                eventData.ContractBaseId,
        ...
        private Task OnDeleted(OtherIncomingDeleted eventData, KafkaMessageValueMetadata eventMetadata)
        {
            ...
            IReadOnlyCollection<BillLink> linkedBills)
        {
            ...
            var baseIds = linkedBills?.Select(link => link.BillBaseId)
