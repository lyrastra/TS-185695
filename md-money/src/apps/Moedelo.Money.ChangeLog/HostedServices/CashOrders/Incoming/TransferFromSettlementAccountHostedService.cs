...
{
    public class TransferFromSettlementAccountHostedService : BackgroundService
    {
        private static TransferFromSettlementAccountStateDefinition StateDefinition => TransferFromSettlementAccountStateDefinition.Instance;

        private readonly ILogger logger;
        private readonly ITransferFromSettlementAccountEventReaderBuilder eventReaderBuilder;
        ...
        public TransferFromSettlementAccountHostedService(
            ILogger<TransferFromSettlementAccountHostedService> logger,
            ITransferFromSettlementAccountEventReaderBuilder eventReaderBuilder,
        ...
        protected override async Task ExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            await eventReaderBuilder
                .OnEvent<TransferFromSettlementAccountCreated>(OnCreated)
                .OnEvent<TransferFromSettlementAccountUpdated>(OnUpdated)
                .OnEvent<TransferFromSettlementAccountDeleted>(OnDeleted)
                .SkipIfUnknownType()
        ...
        private async Task OnCreated(TransferFromSettlementAccountCreated eventData, KafkaMessageValueMetadata eventMetadata)
        {
        ...
        private async Task OnUpdated(TransferFromSettlementAccountUpdated eventData, KafkaMessageValueMetadata eventMetadata)
        {
            var extraInfo = await GetExtraStateInfoAsync(eventData.CashId);
            await changeLogCommandWriter.WriteAsync(
        ...
        private Task OnDeleted(TransferFromSettlementAccountDeleted eventData, KafkaMessageValueMetadata eventMetadata)
        {
