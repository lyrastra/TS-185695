...
namespace Moedelo.Money.ChangeLog.HostedServices.PaymentOrders.Incoming
{
    public class FinancialAssistanceHostedService : BackgroundService
    {
        ...
        private async Task OnCreate(FinancialAssistanceCreatedMessage eventData, KafkaMessageValueMetadata eventMetadata)
        {
        ...
        private async Task OnUpdate(FinancialAssistanceUpdatedMessage eventData, KafkaMessageValueMetadata eventMetadata)
        {
            var extraInfo = await GetExtraStateInfoAsync(eventData.SettlementAccountId);
