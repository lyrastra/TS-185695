...
{
    public class IncomingCurrencySaleHostedService : BackgroundService
    {
        private static IncomingCurrencySaleStateDefinition StateDefinition =>
            IncomingCurrencySaleStateDefinition.Instance;

        private readonly ILogger logger;
        private readonly IIncomingCurrencySaleEventReader eventReader;
        ...
        public IncomingCurrencySaleHostedService(
            ILogger<IncomingCurrencySaleHostedService> logger,
            ISettingRepository settingRepository,
            IIncomingCurrencySaleEventReader eventReader,
        ...
        private async Task OnCreate(IncomingCurrencySaleCreatedMessage eventData, KafkaMessageValueMetadata eventMetadata)
        {
            ...
            var extraInfo = await GetExtraStateInfoAsync(
                ...
                eventData.FromSettlementAccountId ?? 0);
            ...
            await changeLogCommandWriter.WriteAsync(
                ...
                eventData.MapToState(
                    ...
                    extraInfo.FromSettlementAccount),
                eventMetadata.MessageDate);
        }

        private async Task OnUpdate(IncomingCurrencySaleUpdatedMessage eventData, KafkaMessageValueMetadata eventMetadata)
        {
            var extraInfo = await GetExtraStateInfoAsync(
                eventData.SettlementAccountId,
                eventData.FromSettlementAccountId);
        ...
        private Task OnDelete(IncomingCurrencySaleDeletedMessage eventData, KafkaMessageValueMetadata eventMetadata)
        {
            ...
            int fromSettlementAccountId)
        {
            ...
            var settlementAccounts = await settlementAccountApiClient.GetByIdsAsync(
                ...
                new []{settlementAccountId, fromSettlementAccountId});

            return new ExtraStateInfo
            {
                ExecutionContext = executionContext,
                SettlementAccount = settlementAccounts.FirstOrDefault(s => s.Id == settlementAccountId),
                FromSettlementAccount = settlementAccounts.FirstOrDefault(s => s.Id == fromSettlementAccountId)
            };
        }
        ...
        private struct ExtraStateInfo
        {
            ...
            public SettlementAccountDto FromSettlementAccount { get; init; }
        }
    }
}
