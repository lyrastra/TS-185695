...
using Moedelo.Money.Kafka.Abstractions.PaymentOrders.Outgoing.WithdrawalOfProfit;
...
namespace Moedelo.Money.ChangeLog.HostedServices.PaymentOrders.Outgoing
{
    ...
    {
        private static WithdrawalOfProfitStateDefinition StateDefinition =>
            WithdrawalOfProfitStateDefinition.Instance;
        ...
        protected override async Task ExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            await eventReaderBuilder
                .OnEvent<WithdrawalOfProfitCreated>(OnCreated)
                .OnEvent<WithdrawalOfProfitUpdated>(OnUpdated)
                .OnEvent<WithdrawalOfProfitDeleted>(OnDeleted)
                .SkipIfUnknownType()
        ...
        private async Task OnCreated(WithdrawalOfProfitCreated eventData, KafkaMessageValueMetadata eventMetadata)
        {
            ...
            await changeLogCommandWriter.WriteAsync(
                ...
                eventData.MapToState(extraInfo.SettlementAccount),
                eventMetadata.MessageDate);
        }

        private async Task OnUpdated(WithdrawalOfProfitUpdated eventData, KafkaMessageValueMetadata eventMetadata)
        {
            var extraInfo = await GetExtraStateInfoAsync(eventData.SettlementAccountId);
        ...
        private Task OnDeleted(WithdrawalOfProfitDeleted eventData, KafkaMessageValueMetadata eventMetadata)
        {
