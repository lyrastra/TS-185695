...
{
    public class LoanIssueHostedService : BackgroundService
    {
        private static LoanIssueStateDefinition StateDefinition =>
            LoanIssueStateDefinition.Instance;

        private readonly ILogger logger;
        private readonly ILoanIssueEventReaderBuilder eventReaderBuilder;
        ...
        public LoanIssueHostedService(
            ILogger<LoanIssueHostedService> logger,
            ILoanIssueEventReaderBuilder eventReaderBuilder,
        ...
        protected override async Task ExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            await eventReaderBuilder
                .OnEvent<LoanIssueCreated>(OnCreated)
                .OnEvent<LoanIssueUpdated>(OnUpdated)
                .OnEvent<LoanIssueDeleted>(OnDeleted)
                .SkipIfUnknownType()
        ...
        private async Task OnCreated(LoanIssueCreated eventData, KafkaMessageValueMetadata eventMetadata)
        {
        ...
        private async Task OnUpdated(LoanIssueUpdated eventData, KafkaMessageValueMetadata eventMetadata)
        {
            var extraInfo = await GetExtraStateInfoAsync(
                eventData.SettlementAccountId,
                eventData.ContractBaseId);
        ...
        private Task OnDeleted(LoanIssueDeleted eventData, KafkaMessageValueMetadata eventMetadata)
        {
