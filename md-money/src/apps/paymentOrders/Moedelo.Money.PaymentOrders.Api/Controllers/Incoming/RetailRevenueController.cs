...
namespace Moedelo.Money.PaymentOrders.Api.Controllers.Incoming
{
    ...
    public class RetailRevenueController : ControllerBase
    {
        ...
        {
            var model = await reader.GetByBaseIdAsync(documentBaseId, OperationType.MemorialWarrantRetailRevenue).ConfigureAwait(false);
            if (model == null)
            {
                return NotFound();
            }
        ...
        [ProducesResponseType(404)]
        public async Task<IActionResult> CreateAsync(RetailRevenueDto dto)
        {
