...
namespace Moedelo.Money.PaymentOrders.Api.Controllers.Incoming
{
    ...
    public class FinancialAssistanceController : ControllerBase
    {
        ...
        public FinancialAssistanceController(
        ...
        {
            var model = await reader.GetByBaseIdAsync(documentBaseId, OperationType.PaymentOrderIncomingFinancialAssistance).ConfigureAwait(false);
            var data = FinancialAssistanceMapper.Map(model);
        ...
        [ProducesResponseType(404)]
        public async Task<IActionResult> CreateAsync(FinancialAssistanceDto dto)
        {
            var request = FinancialAssistanceMapper.Map(dto);
        ...
        [ProducesResponseType(200)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> UpdateAsync(long documentBaseId, [FromBody] FinancialAssistanceDto dto)
        {
