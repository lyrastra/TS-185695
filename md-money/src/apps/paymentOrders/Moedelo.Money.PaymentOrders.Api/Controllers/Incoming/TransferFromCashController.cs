...
namespace Moedelo.Money.PaymentOrders.Api.Controllers.Incoming
{
    ...
    public class TransferFromCashController : ControllerBase
    {
        ...
        {
            var model = await reader.GetByBaseIdAsync(documentBaseId, OperationType.MemorialWarrantTransferFromCash).ConfigureAwait(false);
            var data = TransferFromCashMapper.Map(model);
        ...
        [ProducesResponseType(404)]
        public async Task<IActionResult> CreateAsync(TransferFromCashDto dto)
        {
            var request = TransferFromCashMapper.Map(dto);
        ...
        [ProducesResponseType(200)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> UpdateAsync(long documentBaseId, [FromBody] TransferFromCashDto dto)
        {
