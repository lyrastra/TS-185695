...
namespace Moedelo.Money.PaymentOrders.Api.Controllers.Incoming
{
    ...
    public class TransferFromCashCollectionController : ControllerBase
    {
        ...
        {
            var model = await reader.GetByBaseIdAsync(documentBaseId, OperationType.MemorialWarrantTransferFromCashCollection).ConfigureAwait(false);
            var data = TransferFromCashCollectionMapper.Map(model);
        ...
        [ProducesResponseType(404)]
        public async Task<IActionResult> CreateAsync(TransferFromCashCollectionDto dto)
        {
            var request = TransferFromCashCollectionMapper.Map(dto);
        ...
        [ProducesResponseType(200)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> UpdateAsync(long documentBaseId, [FromBody] TransferFromCashCollectionDto dto)
        {
