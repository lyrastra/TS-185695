...
namespace Moedelo.Money.PaymentOrders.Api.Controllers.Incoming
{
    [ApiVersion("1.0")]
    [ApiController]
    [Route("private/api/v{version:apiVersion}/Incoming/CurrencyPurchase")]
    public class IncomingCurrencyPurchaseController : ControllerBase
    {
        ...
        {
            var model = await getter.GetByBaseIdAsync(documentBaseId, OperationType.PaymentOrderIncomingCurrencyPurchase).ConfigureAwait(false);
            if (model == null)
            {
                return NotFound();
            }
            var data = IncomingCurrencyPurchaseMapper.Map(model);
        ...
        [ProducesResponseType(404)]
        public async Task<IActionResult> CreateAsync(IncomingCurrencyPurchaseDto dto)
        {
            var request = IncomingCurrencyPurchaseMapper.Map(dto);
        ...
        [ProducesResponseType(200)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> UpdateAsync(long documentBaseId, [FromBody] IncomingCurrencyPurchaseDto dto)
        {
