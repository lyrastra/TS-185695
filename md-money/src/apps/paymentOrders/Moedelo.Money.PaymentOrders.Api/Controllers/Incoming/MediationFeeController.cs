...
namespace Moedelo.Money.PaymentOrders.Api.Controllers.Incoming
{
    ...
    public class MediationFeeController : ControllerBase
    {
        ...
        {
            var model = await reader.GetByBaseIdAsync(documentBaseId, OperationType.PaymentOrderIncomingMediationFee).ConfigureAwait(false);
            var data = MediationFeeMapper.Map(model);
        ...
        [ProducesResponseType(404)]
        public async Task<IActionResult> CreateAsync(MediationFeeDto dto)
        {
            var request = MediationFeeMapper.Map(dto);
        ...
        [ProducesResponseType(200)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> UpdateAsync(long documentBaseId, [FromBody] MediationFeeDto dto)
        {
