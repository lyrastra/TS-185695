...
namespace Moedelo.Money.PaymentOrders.Api.Controllers.Incoming
{
    ...
    public class CurrencyPaymentFromCustomerController : ControllerBase
    {
        ...
        {
            var model = await reader.GetByBaseIdAsync(documentBaseId, OperationType.PaymentOrderIncomingCurrencyPaymentFromCustomer).ConfigureAwait(false);
            if (model == null)
            {
                return NotFound();
            }
            var data = CurrencyPaymentFromCustomerMapper.Map(model);
        ...
        [ProducesResponseType(404)]
        public async Task<IActionResult> CreateAsync(CurrencyPaymentFromCustomerDto dto)
        {
            var request = CurrencyPaymentFromCustomerMapper.Map(dto);
        ...
        [ProducesResponseType(200)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> UpdateAsync(long documentBaseId, [FromBody] CurrencyPaymentFromCustomerDto dto)
        {
