...
namespace Moedelo.Money.PaymentOrders.Api.Controllers.Outgoing
{
    [ApiVersion("1.0")]
    [ApiController]
    [Route("private/api/v{version:apiVersion}/Outgoing/CurrencyPurchase")]
    public class OutgoingCurrencyPurchaseController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetByDocumentBaseIdAsync(long documentBaseId)
        {
            var model = await getter.GetByBaseIdAsync(documentBaseId, OperationType.PaymentOrderOutgoingCurrencyPurchase).ConfigureAwait(false);
            if (model == null)
            {
                return NotFound();
            }
            var data = OutgoingCurrencyPurchaseMapper.Map(model);
        ...
        [ProducesResponseType(404)]
        public async Task<IActionResult> CreateAsync(OutgoingCurrencyPurchaseDto dto)
        {
            var request = OutgoingCurrencyPurchaseMapper.Map(dto);
        ...
        [ProducesResponseType(200)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> UpdateAsync(long documentBaseId, [FromBody] OutgoingCurrencyPurchaseDto dto)
        {
