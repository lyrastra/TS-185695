...
namespace Moedelo.Money.PaymentOrders.Api.Controllers.Outgoing
{
    ...
    public class PaymentToAccountablePersonController : ControllerBase
    {
        ...
        {
            var model = await reader.GetByBaseIdAsync(documentBaseId, OperationType.PaymentOrderOutgoingPaymentToAccountablePerson).ConfigureAwait(false);
            if (model == null)
            {
                return NotFound();
            }
            var data = PaymentToAccountablePersonMapper.Map(model);
        ...
        [ProducesResponseType(404)]
        public async Task<IActionResult> CreateAsync(PaymentToAccountablePersonDto dto)
        {
            var request = PaymentToAccountablePersonMapper.Map(dto);
        ...
        [ProducesResponseType(201)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> CreateWithMissingEmployeeAsync(PaymentToAccountablePersonWithMissingEmployeeDto dto)
        {
            var request = PaymentToAccountablePersonMapper.Map(dto);
            var response = await creator.CreateAsync(request).ConfigureAwait(false);
            return new ApiDataResult(response) { StatusCode = 201 };
        ...
        [ProducesResponseType(200)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> UpdateAsync(long documentBaseId, [FromBody] PaymentToAccountablePersonDto dto)
        {
