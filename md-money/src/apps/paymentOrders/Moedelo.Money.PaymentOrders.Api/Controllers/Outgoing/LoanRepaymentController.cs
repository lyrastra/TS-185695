...
namespace Moedelo.Money.PaymentOrders.Api.Controllers.Outgoing
{
    ...
    public class LoanRepaymentController : ControllerBase
    {
        ...
        {
            var model = await reader.GetByBaseIdAsync(documentBaseId, OperationType.PaymentOrderOutgoingLoanRepayment).ConfigureAwait(false);
            var data = LoanRepaymentMapper.Map(model);
        ...
        [ProducesResponseType(404)]
        public async Task<IActionResult> CreateAsync(LoanRepaymentDto dto)
        {
            var request = LoanRepaymentMapper.Map(dto);
        ...
        [ProducesResponseType(200)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> UpdateAsync(long documentBaseId, [FromBody] LoanRepaymentDto dto)
        {
