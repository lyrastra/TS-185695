...
namespace Moedelo.Money.PaymentOrders.Api.Controllers.Outgoing
{
    ...
    public class RefundToCustomerController : ControllerBase
    {
        ...
        public RefundToCustomerController(
        ...
        {
            var model = await reader.GetByBaseIdAsync(documentBaseId, OperationType.PaymentOrderOutgoingRefundToCustomer).ConfigureAwait(false);
            if (model == null)
            {
                return NotFound();
            }
            var data = RefundToCustomerMapper.Map(model);
        ...
        [ProducesResponseType(404)]
        public async Task<IActionResult> CreateAsync(RefundToCustomerDto dto)
        {
            var request = RefundToCustomerMapper.Map(dto);
        ...
        [ProducesResponseType(200)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> UpdateAsync(long documentBaseId, [FromBody] RefundToCustomerDto dto)
        {
