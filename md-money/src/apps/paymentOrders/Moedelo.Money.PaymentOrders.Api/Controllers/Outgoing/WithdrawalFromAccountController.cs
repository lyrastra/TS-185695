...
namespace Moedelo.Money.PaymentOrders.Api.Controllers.Outgoing
{
    ...
    public class WithdrawalFromAccountController : ControllerBase
    {
        ...
        {
            var model = await reader.GetByBaseIdAsync(documentBaseId, OperationType.MemorialWarrantWithdrawalFromAccount).ConfigureAwait(false);
            var data = WithdrawalFromAccountMapper.Map(model);
        ...
        [ProducesResponseType(404)]
        public async Task<IActionResult> CreateAsync(WithdrawalFromAccountDto dto)
        {
            var request = WithdrawalFromAccountMapper.Map(dto);
        ...
        [ProducesResponseType(200)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> UpdateAsync(long documentBaseId, [FromBody] WithdrawalFromAccountDto dto)
        {
