...
namespace Moedelo.Money.PaymentOrders.Api.Controllers.Outgoing
{
    ...
    public class LoanIssueController : ControllerBase
    {
        ...
        {
            var model = await reader.GetByBaseIdAsync(documentBaseId, OperationType.PaymentOrderOutgoingLoanIssue).ConfigureAwait(false);
            var data = LoanIssueMapper.Map(model);
        ...
        [ProducesResponseType(404)]
        public async Task<IActionResult> CreateAsync(LoanIssueDto dto)
        {
            var request = LoanIssueMapper.Map(dto);
        ...
        [ProducesResponseType(200)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> UpdateAsync(long documentBaseId, [FromBody] LoanIssueDto dto)
        {
