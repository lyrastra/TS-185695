...
namespace Moedelo.Money.PaymentOrders.Api.Controllers.Outgoing
{
    ...
    public class TransferToAccountController : ControllerBase
    {
        ...
        {
            var model = await reader.GetByBaseIdAsync(documentBaseId, OperationType.PaymentOrderOutgoingTransferToAccount).ConfigureAwait(false);
            var data = TransferToAccountMapper.Map(model);
        ...
        [ProducesResponseType(404)]
        public async Task<IActionResult> CreateAsync(TransferToAccountDto dto)
        {
            var request = TransferToAccountMapper.Map(dto);
        ...
        [ProducesResponseType(200)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> UpdateAsync(long documentBaseId, [FromBody] TransferToAccountDto dto)
        {
