...
namespace Moedelo.Money.PaymentOrders.Api.Controllers.Outgoing
{
    [ApiVersion("1.0")]
    [ApiController]
    [Route("private/api/v{version:apiVersion}/Outgoing/CurrencySale")]
    public class OutgoingCurrencySaleController : ControllerBase
    {
        ...
        {
            var model = await getter.GetByBaseIdAsync(documentBaseId, OperationType.PaymentOrderOutgoingCurrencySale).ConfigureAwait(false);
            if (model == null)
            {
                return NotFound();
            }
            var data = OutgoingCurrencySaleMapper.Map(model);
        ...
        [ProducesResponseType(404)]
        public async Task<IActionResult> CreateAsync(OutgoingCurrencySaleDto dto)
        {
            var request = OutgoingCurrencySaleMapper.Map(dto);
        ...
        [ProducesResponseType(200)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> UpdateAsync(long documentBaseId, [FromBody] OutgoingCurrencySaleDto dto)
        {
