...
{
    public class AccrualOfInterestHostedService : BackgroundService
    {
        private readonly ILogger logger;
        private readonly IAccrualOfInterestEventReaderBuilder eventReaderBuilder;
        private readonly IRegistryOperationUpdater registryOperationUpdater;
        private readonly SettingValue consumerCountSetting;

        public AccrualOfInterestHostedService(
            ILogger<AccrualOfInterestHostedService> logger,
            ISettingRepository settingRepository,
            IAccrualOfInterestEventReaderBuilder eventReaderBuilder,
        ...
        protected override Task ExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            return eventReaderBuilder
                .OnEvent<AccrualOfInterestCreated>(OnCreatedAsync)
                .OnEvent<AccrualOfInterestUpdated>(OnUpdatedAsync)
                .OnEvent<AccrualOfInterestDeleted>(OnDeletedAsync)
                .SkipIfUnknownType()
        ...
        private Task OnCreatedAsync(AccrualOfInterestCreated eventData, KafkaMessageValueMetadata eventMetadata)
        {
        ...
        private Task OnUpdatedAsync(AccrualOfInterestUpdated eventData, KafkaMessageValueMetadata eventMetadata)
        {
            return registryOperationUpdater.UpdateAsync(
                eventData.MapToCommand());
        }

        private Task OnDeletedAsync(AccrualOfInterestDeleted eventData, KafkaMessageValueMetadata eventMetadata)
        {
