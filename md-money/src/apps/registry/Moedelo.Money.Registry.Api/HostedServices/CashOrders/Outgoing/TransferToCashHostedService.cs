    ...
    public class TransferToCashHostedService : BackgroundService
    {
        ...
        protected override Task ExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            return eventReaderBuilder
                .OnEvent<TransferToCashCreated>(OnCreatedAsync)
                .OnEvent<TransferToCashUpdated>(OnUpdatedAsync)
                .OnEvent<TransferToCashDeleted>(OnDeletedAsync)
                .SkipIfUnknownType()
        ...
        private Task OnCreatedAsync(TransferToCashCreated eventData, KafkaMessageValueMetadata eventMetadata)
        {
            return registryOperationUpdater.CreateAsync(
                eventData.MapToCommand());
        }

        private Task OnUpdatedAsync(TransferToCashUpdated eventData, KafkaMessageValueMetadata eventMetadata)
        {
            return registryOperationUpdater.UpdateAsync(
                eventData.MapToCommand());
        }

        private Task OnDeletedAsync(TransferToCashDeleted eventData, KafkaMessageValueMetadata eventMetadata)
        {
