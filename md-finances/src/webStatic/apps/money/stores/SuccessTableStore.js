...
class SuccessTableStore {
    ...
    @action loadOperations = async (options = { reset: false }) => {
        ...
        const { Operations, TotalCount } = await OperationTablesService.getSuccessOperationsList(offset, count, this.sourceId, this.sourceType);

        let successOperations = Operations;
        ...
        if (this.isOutsourceUser) {
            ...
            const Ids = successOperations.reduce((acc, item) => {
            ...
            successOperations = successOperations.map(operation => {
        ...
        runInAction(() => {
            const parsedOperations = this.parseOperationsToModels(successOperations);
            ...
            this.loaded = !!this.operations.length;
            this.loading = false;
        });
    }
