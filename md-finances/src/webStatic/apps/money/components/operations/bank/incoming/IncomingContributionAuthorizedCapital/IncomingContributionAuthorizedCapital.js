...
import IncomingContributionAuthorizedCapitalStore from './stores/IncomingContributionAuthorizedCapitalStore';
...
@observer
class IncomingContributionAuthorizedCapital extends React.Component {
    constructor(props) {
        super(props);

        this.store = new IncomingContributionAuthorizedCapitalStore({
    ...
    getKontragentTooltip = () => {
        const content = (
            <div>Если учредитель еще не заведен в системе, то создайте его в разделе&nbsp;
                <a
                    target={`_blank`}
                    href={getUrlWithId(`/Estate/Home#/authorizedCapital/`)}
                >
                    Уставный капитал
                </a>
            </div>
...
IncomingContributionAuthorizedCapital.propTypes = {
...
export default IncomingContributionAuthorizedCapital;
