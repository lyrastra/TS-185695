...
import PaymentToSupplierStore from './PaymentToSupplierStore';
...
describe(`PaymentToSupplier test`, () => {
    beforeEach(() => {
        setupFetch([
            {
    ...
    it(`set sum - keep documents for payment sum only`, () => {
        const store = createStore({
            operation: {
                Documents: [{
                    DocumentSum: 100, PaidSum: 0, key: 1, Sum: 90
                }, { Sum: 10, DocumentSum: 100, key: 2 }],
                Sum: 100
            }
        });
...
function createStore(operationData) {
    return new PaymentToSupplierStore(getOperationDataForTest(operationData));
}
