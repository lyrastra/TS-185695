...
const availableContractKinds = [
    ContractKind.ReceivedCredit,
    ContractKind.ReceivedLoan
];

class LoanRepaymentStore extends Actions {
    ...
    constructor(options) {
        ...
        if (!availableContractKinds.includes(this.model.Contract?.ProjectKind)) {
            this.setContract();
        }
    ...
    initReactions = () => {
        ...
        reaction(() => [this.model.Kontragent, this.model.Sum, this.model.LoanInterestSum], this.handleDescriptionMessage);
    ...
    handleDescriptionMessage = () => {
        ...
        const { Sum, Kontragent, LoanInterestSum } = this.model;
        const kontragentName = Kontragent.KontragentName && Kontragent.KontragentName.length > 0 ? ` ${Kontragent.KontragentName}` : ``;
        const sum = Sum > 0 ? ` на сумму ${toAmountString(Sum)} руб` : ``;
        const loanSum = LoanInterestSum > 0 ? `, в том числе процентов на сумму ${toAmountString(LoanInterestSum)} руб` : ``;

        this.setDescription(`Погашение займа ${sum}${loanSum} контрагенту${kontragentName}. НДС не облагается.`);
    };
    ...
    getContractAutocomplete = async ({ query = `` }) => {
        const response = await autocomplete({
            ...
            kind: [ContractKind.ReceivedCredit, ContractKind.ReceivedLoan] // договоры займа и кредита
        });
    ...
    modelForSave() {
        ...
        const operationType = paymentOrderOperationResources.PaymentOrderOutgoingLoanRepayment;
...
export default LoanRepaymentStore;
