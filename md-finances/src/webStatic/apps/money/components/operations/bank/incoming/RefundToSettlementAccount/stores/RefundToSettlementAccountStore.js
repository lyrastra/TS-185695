import {
    observable, reaction, toJS, runInAction, makeObservable
} from 'mobx';
...
import { initCreditSubcontos } from '../refundToSettlementAccountSubcontoService';

class RefundToSettlementAccountStore extends Actions {
    ...
    constructor(options) {
        ...
        if (typeof this.model.IncludeNds !== `boolean`) {
            this.setIncludeNds({ checked: this.TaxationSystem.IsOsno });
        }
    ...
    modelForSave = () => {
        ...
        const operationType = MoneyOperationTypeResources.PaymentOrderRefundToSettlementAccount;
    ...
    loadAccountingPostings = async () => {
        ...
        if (this.model.AccountingPostings.Postings || !this.model.AccountingPostings?.Postings?.length) {
            ...
            const syntheticAccountCodes = await getSyntheticAccountAutocomplete();
            const debit = syntheticAccountCodes.find(item => item.Code === SyntheticAccountCodesEnum._51_01);
            debit.ReadOnly = true;
            ...
            // если именно контрагент, а не сотрудник и т.д., предзаполняем кредит и объект учета
            if (this.model.Kontragent.KontragentId) {
                posting.Credit = syntheticAccountCodes.find(item => item.Code === SyntheticAccountCodesEnum._60_02);
                posting.SubcontoCredit = await initCreditSubcontos({ model: this.model, credit: posting.Credit });
            }
    ...
    initAccountingPostings = () => {
        if (this.model.Id > 0 && Object.keys(this.model.AccountingPostings).length) {
...
export default RefundToSettlementAccountStore;
