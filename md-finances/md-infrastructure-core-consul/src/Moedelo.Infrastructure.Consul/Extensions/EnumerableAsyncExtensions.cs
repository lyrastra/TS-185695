...
internal static class EnumerableAsyncExtensions
{
    ...
    internal static async Task<List<TResult>> SelectParallelAsync<TResult, TSource>(
                        ...
                        using (partition)
                        {
                            while (!cancellationToken.IsCancellationRequested && partition.MoveNext())
                            {
                                try
                                {
                                    ...
                                    results[index] = await taskFactory(item, cancellationToken).ConfigureAwait(false);
                                }
                                catch (Exception e)
                                {
                                    exceptions.Add(e);
