...
namespace Moedelo.PaymentOrderImport.ApiClient.PaymentOrderMlOperationType
{
    ...
    public class PaymentOrderMlOperationTypeApiClient : BaseApiClient, IPaymentOrderMlOperationTypeApiClient
    {
        ...
        public async Task<VerifyOperationTypeResponseDto> VerifyAsync(
            VerifyOperationTypeDto request,
            CancellationToken ct)
        {

            var result = await PostAsync<VerifyOperationTypeDto, ApiDataDto<VerifyOperationTypeResponseDto>>(
        ...
        {
            return PostAsync(
                "/private/api/v1/PaymentOrderMlOperationType/SetSkipStatus", request, cancellationToken: ct);
        }
    }
}
