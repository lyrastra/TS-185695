...
namespace Moedelo.Docs.Client.SalesBills
{
    [InjectAsSingleton]
    public class SalesBillsApiClient : BaseApiClient, ISalesBillsApiClient
    {
        ...
        public Task<List<PaidSumDto>> GetPaidSumByBaseIdsAsync(int firmId, int userId, IReadOnlyCollection<long> baseIds)
        {
            ...
            return PostAsync<IReadOnlyCollection<long>, List<PaidSumDto>>($"/SalesBills/GetPaidSumByBaseIds?firmId={firmId}&userId={userId}", baseIds);
        }
    }
}