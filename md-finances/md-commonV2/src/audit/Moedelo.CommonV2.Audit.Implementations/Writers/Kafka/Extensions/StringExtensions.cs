...
namespace Moedelo.CommonV2.Audit.Writers.Kafka.Extensions
{
    internal static class StringExtensions
    {
        ...
        private static readonly string[] SensitiveJsonProperties =
            new [] { "password", "passwordmd5", "publictoken", "token", "apikey", "access_token", "refresh_token" };

        /// <summary>
        /// Название cвойства, которые должны быть маскированы (не только как json-свойства)
        /// </summary>
        ...
        {
            yield return new(new Regex($@"\b(http|https|mongodb)://(\w+):(\w+)@", RegExOptions, RegExMaxTimeout),
                @"$1://***:***@");
        }
